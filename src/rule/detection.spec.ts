...
describe("Detection", () => {
  ...
  test('"N of them" statement should be expanded correctly', () => {
    ...
    expect(detection.expandCondition()).toEqual(
      "((cond1 and cond2) or (cond1 and cond3) or (cond2 and cond3))"
    );
  });
  ...
  test('"N of condition" statement should be expanded correctly', () => {
    const detection: Detection = new Detection();
    detection.cond1 = {
      sub1: { process: "test1.exe" },
      sub2: { process: "test1.exe" },
    };
    detection.cond2 = {
      sub1: { process: "test2.exe" },
      sub2: { process: "test2.exe" },
    };
    detection.cond3 = {
      sub1: { process: "test3.exe" },
      sub2: { process: "test3.exe" },
    };
    detection.condition = "(2 of cond1) or (1 of cond2)";

    expect(detection.expandCondition()).toEqual(
      "((cond1.sub1 and cond1.sub2)) or (((cond2.sub1) or (cond2.sub2)))"
    );
  });
