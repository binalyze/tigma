...
export class SigmaScanner implements ISigmaScanner {
  ...
  private scanRule(rule: SigmaRule, json: ObjectLiteral): boolean {
    ...
    this.logger.debug(
      `Expression: ${rule.detection.condition}, Expanded: ${expandedCondition}, Conditions: ${conditions}`
    );

    const options = {
      extraFunctions: {
        evaluateCondition: (conditionName: string) =>
          self.evaluateCondition(rule, conditionName, json),
      },
    };
    ...
    for (let i in conditions) {
      ...
      if (reGroupCondition.test(lazyConditionExpression)) {
        ...
        matches.forEach((m: string) => {
          lazyConditionExpression = lazyConditionExpression.replace(
            m,
            `evaluateCondition("${m}")`
          );
        });
    ...
    this.logger.debug(
      `Re-written rule for lazy evaluation: ${lazyConditionExpression}`
    );

    if (lazyConditionExpression.indexOf("evaluateCondition") === -1) {
      this.logger.error(
        `Lazy condition looks erroneous due to the lack of lazy evaluation function: ${lazyConditionExpression}`
      );
      return false;
    }
  ...
  private evaluateCondition(
    ...
    this.logger.debug(
      `Identifiers: ${JSON.stringify(identifierTree, null, 2)}`
    );
  ...
  private matchCondition(
    ...
    for (let id in condition.values) { // Example: Condition is "selection" whereas values are Processes, Prefetch and etc.
      ...
      this.logger.debug(
        `Condition section ${sectionIdentifier.name} match result: ${matched}`
      );
  ...
  private matchString(
    ...
    if (this.getModifier(modifiers, ModifierType.Contains) != null) {
      ...
      this.getModifier(modifiers, ModifierType.Equals) != null ||
  ...
  private matchNumber(
    ...
    if (this.getModifier(modifiers, ModifierType.LessThan) != null) {
      ...
      this.getModifier(modifiers, ModifierType.LessThanOrEqual) != null
    ) {
      matched = target <= source;
    } else if (this.getModifier(modifiers, ModifierType.GreaterThan) != null) {
      matched = target > source;
    } else if (
      this.getModifier(modifiers, ModifierType.GreaterThanOrEqual) != null
    ) {
      matched = target >= source;
    } else {
      matched = source === target;
  ...
  private matchPrimitive(
    ...
    if (sourceType === targetType) {
      ...
      if (sourceType === "boolean") {
        matched = source ? target > 0 : target === 0;
  ...
  private filterByIdentifier(
    ...
    if (!json) {
      this.logger.debug(
        `Undefined target provided for matching with identifier ${JSON.stringify(
          identifier
        )}`
      );
      return false;
    ...
    this.logger.debug(
      `Matching identifier ${identifier.name} on target: ${JSON.stringify(
        json
      )}`
    );
    ...
    for (let i in identifier.values) {
      ...
      this.logger.debug(
        `Matching sub-identifier ${
          subIdentifier.name
        } on target: ${JSON.stringify(json)}`
      );
      ...
      if (subIdentifier.type === IdentifierType.Primitive) {
        ...
        matched = this.filterByIdentifier(
          jsonElement,
          subIdentifier,
          depth + 1
        );
      ...
      if (!matched) {
        ...
        //
        if (
          this.getModifier(identifier.modifiers, ModifierType.All) ||
